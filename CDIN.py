...
class CDIN:
    ...
    def remove_columns(dataframe, columnas_a_eliminar):
        #Elimina las columnas especificadas de un DataFrame con el drop
        #dataframe (pd.DataFrame): El DataFrame del cual eliminar las columnas.
        #columnas_a_eliminar (list): Lista de nombres de columnas a eliminar.
        return dataframe.drop(columnas_a_eliminar, axis=1)
    
    def redondear_numero(numero, decimales=0):
    
    #Redondea un número al número especificado de decimales.
    
    #Args:
    #numero (float): El número que se va a redondear.
    #decimales (int): La cantidad de decimales a la que se redondeará el número. Por defecto, es 0.
